...
def simulate_rf_transmit(
    ...
    n_frames = scatterer_amplitudes.shape[0]

    el_indices = device_put(el_indices, device=device)
    ax_indices = device_put(ax_indices, device=device)
    scatterer_amplitudes = device_put(scatterer_amplitudes, device=device)
    scatterer_positions = device_put(scatterer_positions, device=device)

    progbar_func = lambda x, **kwargs: (x if not progress_bar else tqdm(x, **kwargs))

    rf_data = []
    for frame in progbar_func(
        range(0, n_frames), desc="Frame", leave=False
    ):
        amplitudes_frame = scatterer_amplitudes[frame]
        rf_data_frame = simulation_function(ax_indices, el_indices, scatterer_positions, scatterer_amplitudes)
        rf_data.append(rf_data_frame)
...
def simulate_partial_rf_data(
    ...
    rf_data = jnp.stack(rf_transmits, axis=1)
